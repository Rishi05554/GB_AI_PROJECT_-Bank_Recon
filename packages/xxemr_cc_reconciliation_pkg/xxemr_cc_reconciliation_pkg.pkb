...
-- Writes only to xxemr_match_groups / xxemr_match_group_details
-- ================================================================
        ...
        IF l_statement_line_id IS NOT NULL AND g.unmatched_cnt = 0 THEN
            DELETE FROM xxemr_match_group_details
             WHERE match_group_id IN (
                       SELECT mg.match_group_id
                         FROM xxemr_match_groups mg
                        WHERE mg.statement_line_id = l_statement_line_id
                          AND (
                               mg.match_type = 'NO_MATCH_FOUND'
                               OR NVL(mg.user_action, 'PENDING') = 'NEVER_PROCESSED'
                               OR mg.match_type = 'CC_MATCH'
                          )
                   );
            ...
            DELETE FROM xxemr_match_groups
             WHERE statement_line_id = l_statement_line_id
               AND (
                    ...
                    OR NVL(user_action, 'PENDING') = 'NEVER_PROCESSED'
                    OR match_type = 'CC_MATCH'
               );
            ...
            INSERT INTO xxemr_match_groups (
                ...
                created_by,
                fusion_recon_status,
                application_recon_status,
                approve_action
            ) VALUES (
                xxemr_match_groups_seq.NEXTVAL,
                l_statement_line_id,
                g.matched_sum,
                0,
                100,
            ...
            FOR r IN (
                ...
                INSERT INTO xxemr_match_group_details (
                    ...
                    xxemr_match_grp_dtl_seq.NEXTVAL,
                    l_match_group_id,
                    r.cash_receipt_id,
                    r.receipt_amount,
                    100,
            ...
            log_cc('CC_MATCH created: Bank=' || g.bank_account_id
                || ' | Sum=' || g.matched_sum
                || ' | Receipts=' || g.matched_cnt
...
-- Reads from xxemr_match_groups / details and stages to cc_fbdi_lines
-- cc_match_group_id stores main match_group_id
-- ================================================================
...
BEGIN
    log_cc('xxemr_build_cc_fbdi START. upload_id=' || p_upload_id
        || ' | match_group_id=' || NVL(TO_CHAR(p_match_group_id), 'ALL'));

    IF p_match_group_id IS NULL THEN
        DELETE FROM xxemr_cc_fbdi_lines
         WHERE upload_id = p_upload_id;
    ELSE
        DELETE FROM xxemr_cc_fbdi_lines
         WHERE upload_id = p_upload_id
           AND cc_match_group_id = p_match_group_id;
    END IF;

    FOR g IN (
        SELECT mg.match_group_id,
               mg.statement_line_id,
               mg.total_match_amount AS statement_amount,
               s.bank_account_id
          FROM xxemr_match_groups mg
          JOIN xxemr_bank_statement_lines s
            ON s.statement_line_id = mg.statement_line_id
         WHERE mg.match_type = 'CC_MATCH'
           AND NVL(mg.user_action, 'PENDING') IN ('PENDING','ACCEPTED','ACTION_TAKEN')
           AND (p_match_group_id IS NULL OR mg.match_group_id = p_match_group_id)
           AND EXISTS (
               SELECT 1
                 FROM xxemr_cc_fund_transfer ft
                WHERE ft.upload_id       = p_upload_id
                  AND ft.bank_account_id = s.bank_account_id
                  AND ft.match_status    = 'MATCHED'
           )
    ) LOOP
        BEGIN
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                log_cc('WARNING: header data not found for stmt_line=' || g.statement_line_id);
                CONTINUE;
        END;
        ...
        INSERT INTO xxemr_cc_fbdi_lines (
            ...
            xxemr_cc_fbdi_seq.NEXTVAL, g.match_group_id, p_upload_id,
        ...
        FOR r IN (
            SELECT mgd.candidate_id AS cash_receipt_id,
                   mgd.amount       AS receipt_amount,
                   ar.receipt_number
              FROM xxemr_match_group_details mgd
              JOIN xxemr_ar_cash_receipts ar
                ON ar.cash_receipt_id = mgd.candidate_id
             WHERE mgd.match_group_id   = g.match_group_id
               AND mgd.candidate_source = 'AR'
             ORDER BY mgd.match_group_detail_id
        ) LOOP
            INSERT INTO xxemr_cc_fbdi_lines (
                cc_fbdi_line_id, cc_match_group_id, upload_id,
                statement_number, bank_acct_internal_id,
                ...
                r.receipt_amount, l_statement_date,
                'PENDING', 0,
                p_created_by, SYSTIMESTAMP,
                p_created_by, SYSTIMESTAMP
            );
...
BEGIN
    log_cc('xxemr_dispatch_cc_to_oic START. upload_id=' || NVL(TO_CHAR(p_upload_id),'ALL'));
    ...
    SELECT COUNT(DISTINCT cc_match_group_id)
     ...
     WHERE send_status  IN ('PENDING','FAILED')
	   ...
	   AND (p_match_group_id IS NULL OR cc_match_group_id = p_match_group_id);
    ...
    FOR g IN (
        ...
        l_match_group_id := g.cc_match_group_id;
        l_id_count := 0;
        l_id_list.DELETE;
        ...
        BEGIN
            ...
            DBMS_LOB.APPEND(l_payload, TO_CLOB(
                '{' ||
                '"statementNumber":"'      || json_str(l_neg.statement_number) || '",' ||
                ...
                '"negLine":{' ||
                    ...
                    '"amount":'    || TO_CHAR(l_neg.amount, 'FM999999999999990.00') ||
                '},' ||
                '"lines":['
            ));
        ...
        FOR r IN (
            ...
            DBMS_LOB.APPEND(l_payload, TO_CLOB(
                ...
                '"amount":'         || TO_CHAR(r.amount, 'FM999999999999990.00') || ',' ||
        ...
        IF l_api_status IN (200, 201, 202) THEN
            ...
            UPDATE xxemr_match_groups
               SET user_action        = 'ACTION_TAKEN',
                   application_recon_status = 'SUBMITTED',
                   last_updated_by    = 'CC_DISPATCHER',
                   last_update_date   = SYSTIMESTAMP
            ...
            FORALL i IN 1..l_id_count
                UPDATE xxemr_cc_fbdi_lines
                   SET send_status = CASE
                                         WHEN send_attempts >= p_max_attempts THEN 'ERROR'
                                         ELSE 'FAILED'
                                     END
                 WHERE cc_fbdi_line_id = l_id_list(i);
    ...
    log_cc('xxemr_dispatch_cc_to_oic DONE. Sent=' || l_sent_count || ' Failed=' || l_failed_count);
...
-- ================================================================
-- STEP 6: xxemr_run_cc_keyword_check
-- ================================================================
...
IS
    ...
    l_ft_updated NUMBER;
BEGIN
    log_cc('xxemr_run_cc_keyword_check START. bank=' || NVL(TO_CHAR(p_bank_account_id),'ALL'));
    ...
    COMMIT;
    log_cc('xxemr_run_cc_keyword_check DONE. CC flagged=' || l_cc_updated
...
-- ================================================================
-- MASTER: process upload only, no auto submit
-- ================================================================
...
BEGIN
    ...
    log_cc('CC upload processed. Matches are now available in XXEMR_MATCH_GROUPS for user review.');
...
-- ================================================================
-- Submit after user review
-- ================================================================
PROCEDURE xxemr_submit_cc_upload (
    p_upload_id       IN NUMBER,
    p_match_group_id  IN NUMBER   DEFAULT NULL,
    p_created_by      IN VARCHAR2 DEFAULT 'SYSTEM'
)
...
BEGIN
    log_cc('========================================');
    log_cc('xxemr_submit_cc_upload START.'
        || ' upload_id='      || p_upload_id
        || ' | match_group='  || NVL(TO_CHAR(p_match_group_id), 'ALL'));
    log_cc('========================================');
    ...
    xxemr_dispatch_cc_to_oic(
        p_upload_id      => p_upload_id,
        p_match_group_id => p_match_group_id
    );

    log_cc('========================================');
    log_cc('xxemr_submit_cc_upload DONE.');
    log_cc('========================================');

EXCEPTION
    WHEN OTHERS THEN
        log_cc('FATAL: xxemr_submit_cc_upload: ' || SQLERRM
            || CHR(10) || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
        RAISE;
END xxemr_submit_cc_upload;
